 ...
 */
export function compileSingleNumberArgmentFunction(ctx, inputState, callExpr, actions) {
    if (callExpr.args.length !== 1) {
        throw Error(`${callExpr.name} argments length is not 1`);
    }
    ...
    if (arg.value < 0) {
        throw Error(`${callExpr.name} argment is negtive`);
    }
