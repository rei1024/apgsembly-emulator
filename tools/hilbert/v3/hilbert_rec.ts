...
// 内部で使う、トポロジー（反転・回転状態）の定義
type RuleType = "A" | "B";

/**
 * 完全に無限に続くヒルベルト曲線のコマンドストリーム
 */
export function* generateInfiniteHilbert(): Generator<
  HilbertCommand,
  void,
  unknown
> {
...
): Generator<HilbertCommand, void, unknown> {
 ...
 */
function* runA(depth: number): Generator<HilbertCommand, void, unknown> {
  if (depth === 0) return;
  yield "-";
  yield* runB(depth - 1);
  yield "F";
...
function* runB(depth: number): Generator<HilbertCommand, void, unknown> {
  if (depth === 0) return;
  yield "+";
  yield* runA(depth - 1);
  yield "F";
