...
export function convert(tmMap) {
    ...
    for (const state of tmMap.states) {
        ...
        // 空白の場合
        {
            ...
            if (
                blankLine.newSymbol === undefined ||
                blankLine.newSymbol === BLANK_SYMBOL
            ) {
                // 空白
                array.push(
            ...
            if (blankLine.newState === undefined) {
                array.push(
                    `${state}__Z_FINISH_1; *; ${state}__CHECK_SYMBOL_1; NOP`,
                );
            } else {
                array.push(
                    `${state}__Z_FINISH_1; *; ${blankLine.newState}__CHECK_SYMBOL_1; NOP`,
                );
            }
        }
        ...
        // 空白でない場合
        {
            ...
            if (otherLine.newState === undefined) {
                array.push(
                    `${state}__NZ_FINISH_1; *; ${state}__CHECK_SYMBOL_1; NOP`,
                );
            } else {
                array.push(
                    `${state}__NZ_FINISH_1; *; ${otherLine.newState}__CHECK_SYMBOL_1; NOP`,
                );
            }
        }
    }
